...
namespace transform {
    namespace {
        double rand_double() { return torch::rand(1)[0].item<double>(); }

        int64_t rand_int(int64_t max) { return torch::randint(max, 1)[0].item<int64_t>(); }
    } // namespace
    ...
    ConstantPad::ConstantPad(std::vector<int64_t> padding, torch::Scalar value)
    ...
    RandomCrop::RandomCrop(std::vector<int64_t> size) : size_(std::move(size)) {}
