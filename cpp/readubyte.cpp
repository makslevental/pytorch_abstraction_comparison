...
size_t ReadUByteDataset(
    ...
    if (image_header.length != label_header.length) {
        printf("ERROR: Dataset file mismatch (number of images does not match the number of "
               "labels)\n");
        fclose(imfp);
        fclose(lbfp);
        return 0;
    ...
    if (data != nullptr) {
        if (fread(data, sizeof(uint8_t), image_header.length * width * height, imfp) !=
