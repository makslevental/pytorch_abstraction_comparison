...
void Layer::bwd_initialize(Tensor<double> *grad_of_output) {
    if (grad_of_input_ == nullptr || batch_size_ != grad_of_output->get_batch_size()) {
        grad_of_output_ = grad_of_output;

        if (grad_of_input_ == nullptr)
            grad_of_input_ = new Tensor<double>(input_->shape());
        else
